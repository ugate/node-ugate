...
* (Optional) `ssh` is enabled on C.H.I.P. by default, but another great option is to install a [VNC server](https://en.wikipedia.org/wiki/Virtual_Network_Computing) so you have remote access to the C.H.I.P. GUI. [TightVNC](http://www.tightvnc.com/) seems to be a popular VNC client/server. To install the server on C.H.I.P. `sudo apt-get install tightvncserver`then [install the client](http://www.tightvnc.com/download.php) on your remote computer.
